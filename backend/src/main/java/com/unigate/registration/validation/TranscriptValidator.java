...
@Component
public class TranscriptValidator implements DocumentValidator {

    private static final long MAX_SIZE_BYTES = 5 * 1024 * 1024;

    @Override
    public ValidationResult validate(Document document) {
        if (document.getType() != DocumentType.TRANSCRIPT) {
        ...
        if (document.getFileName() == null || document.getFileName().isBlank()) {
            return ValidationResult.fail("Transcript file name is missing");
        }
        String lower = document.getFileName().toLowerCase();
        if (!lower.endsWith(".pdf")) {
            return ValidationResult.fail("Transcript must be a PDF file");
        }
        if (document.getFileSize() != null && document.getFileSize() > MAX_SIZE_BYTES) {
            return ValidationResult.fail("Transcript file exceeds 5 MB limit");
        }
